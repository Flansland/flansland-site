...
const helpSearchResult = document.querySelector("#help-search-result");
if (helpPanel && helpToggle) {
  helpToggle.addEventListener("click", () => {
    ...
    helpToggle.setAttribute("aria-label", isOpen ? "Abrir central de ajuda" : "Fechar central de ajuda");
  });
}
if (helpSearch && helpSearchResult) {
  helpSearch.addEventListener("input", () => {
    const query = helpSearch.value.trim();
    helpSearchResult.hidden = !query;
    helpSearchResult.textContent = query ? `Ainda não encontramos uma resposta para “${query}”. Tente iniciar uma conversa.` : "";
  });
}
